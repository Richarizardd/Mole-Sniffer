...
public class Polygon {
    ...
    public static class Builder {
        private List<PointFloat> _vertexes = new ArrayList<PointFloat>();
        private List<Line> _sides = new ArrayList<Line>();
        private BoundingBox _boundingBox = null;

        private boolean _firstPointFloat = true;
         ...
         * @return The builder
         */
        public Builder addVertex(PointFloat point) {
            if (_isClosed) {
                // each hole we start with the new array of vertex points
                _vertexes = new ArrayList<PointFloat>();
                _isClosed = false;
            }
         ...
         */
        private void updateBoundingBox(PointFloat point) {
            if (_firstPointFloat) {
     ...
     */
    public boolean contains(PointFloat point) {
    ...
    private boolean intersect(Line ray, Line side) {
        PointFloat intersectPointFloat = null;
        ...
        if (!ray.isVertical() && !side.isVertical()) {
            ...
            intersectPointFloat = new PointFloat(x, y);
        ...
        // System.out.println("Intersect point: " + intersectPointFloat.toString());

        if (side.isInside(intersectPointFloat) && ray.isInside(intersectPointFloat)) {
            return true;
        }
     ...
     * @return
     */
    private Line createRay(PointFloat point) {
        // create outside point
        float epsilon = (_boundingBox.xMax - _boundingBox.xMin) / 100f;
        PointFloat outsidePointFloat = new PointFloat(_boundingBox.xMin - epsilon, _boundingBox.yMin);

        Line vector = new Line(outsidePointFloat, point);
        return vector;
    }
     ...
     */
    private boolean inBoundingBox(PointFloat point) {
